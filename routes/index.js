...
router.get('/database/:collectionName',async function(req, res, next) {
  if(req.params.collectionName=="Yeni_Yığın")
    res.render('collectionViewer', { title: 'Yeni Yığın' ,url:req.url,method:"create",collection:`$('[data-name="yığınAdı"]').val()` });
  else if(req.params.collectionName=="Yığını_Düzenle"){
    if(req.query.name!=undefined){
      const db = req.app.locals.db;
      _options=(await db.command({'listCollections': 1 ,"filter":{"name":"mycol"}})).cursor.firstBatch[0].options
...
router.post('/changeCollection',async function(req, res, next){
  ...
  switch (_data.method) {
    ...
    case "create":
      ...
      status=(await db.createCollection(_data.collectionName,_data.options)).result;
      text = "Oluşturuldu!";
      renk="success"
      break;
    default:
