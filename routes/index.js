...
router.get('/duyuru', async function (req, res, next) {
  const db = req.app.locals.db;
  var _data = await db.collection("Duyurular").find().sort({
    date: -1
  }).toArray();
  ...
  if (req.params.collectionName == "Yeni_Yığın")
    ...
    if (req.query.name != undefined) {
      ...
      res.render('error', {
        ...
        error: {
          status: "404",
          stack: "Bilgileriniz kontrol edip tekrar deneyiniz!"
        }
      });
  } else {
    const db = req.app.locals.db;
...
router.get('/database/:collectionName/:id', async function (req, res, next) {
  ...
  if (req.params.id != "Yeni_Kayıt" && req.params.collectionName != "Yeni_Yığın") {
    _data = await db.collection(req.params.collectionName).findOne({
...
router.get('/Form/:pageName/:id', async function (req, res, next) {
  ...
  if (req.params.id == "Yeni_Kayıt") {
    ...
    _data = await db.collection(req.params.pageName).findOne({
      '_id': ObjectId(req.params.id)
    });
    pages = (await db.collection("Sayfalar").findOne({
      'pageName': req.params.pageName
...
router.post('/ajax/login', async function (req, res, next) {
  ...
  if (_data == undefined || _data.userName == "" || _data.password == "") {
    ...
    _user = (await db.collection("Kullanıcılar").findOne({
...
router.post('/ajax/changeDocument', async function (req, res, next) {
  ...
  try {
    ...
    link = (await db.collection("Sayfalar").findOne({
    ...
    switch (_data.method) {
      case "update":
        (await db.collection(_data.collection).deleteOne({
        ...
        text = "Güncellendi!";
        status.ok = 1
        business.addNotification(db, _data.method, _data.collection, link + '/' + _data.id, {
          xx: "hello"
        });
        break;
      case "delete":
        ...
        text = "Silindi!";
        status.ok = 1
        business.addNotification(db, _data.method, _data.collection, link, {
          xx: "hello"
        });
        break;
      case "create":
        ...
        status.ok = 1;
        text = "Oluşturuldu!";
        business.addNotification(db, _data.method, _data.collection, link + '/' + r.insertedId, {
          xx: "hello"
        });
        break;
      default:
  ...
  if (status.ok == undefined || status.ok != 1) {
    ...
    (await db.collection("Kullanıcılar").updateOne({
...
router.post('/ajax/filter', async function (req, res, next) {
  ...
  try {
    ...
    await business.checkAllow(req.session.user, db, _collectionName);
