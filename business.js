...
/* #region  exports */
module.exports = {
    async inputGenerator(_array, _db) {
        var _txt = "";
        var _contentArray = {};
        ...
        for (item of _array) {
            ...
            if (item.type == "select") {
                ...
                if (item.special != undefined && item.special != "") {
                    ...
                    selectItems = (await _db.collection("Sabit Seçim Verileri").findOne({
                        "name": item.fixedData
                    })).content;
                } else if (item.target != undefined && item.target != "") {
                    var tmp = await _db.collection(item.target).find({}).toArray();
                    var re = (await _db.collection("Sayfalar").findOne({
                ...
                if (item.special != undefined && item.special != "") {
                    switch (item.special) {
                        case "subPage":
                            ...
                            var c = (await _db.collection("Sayfalar").findOne({
                                'collection': 'Sayfalar'
                            })).content;
                            arr = (c.find(x => x.key === 'content').items).filter(x => x.special != 'subPage' || x.special == undefined);
                            tmpObj = (await this.inputGenerator(arr, _db));
                            _contentArray = kk;
                ...
                for (val of tmp.value) {
                    _arr =await this.setValuesToinputs(arr, val);
                    _fillArrayItems += `
                        <div class="row pb-1 pt-2 gainsboro card col-md-${item.size}" index="${i}"  >
                            ...
                            ${(await this.inputGenerator(_arr,_db)).txt}
                        </div>
                        `;
                    i++;
                }
    ...
    async viewGenerator(_page, _db, _url, _query) {
        var _txt = ""
    
        //header
        header = await this.viewHeaderGenerator(_page, _db, _url);
        //body
        bodyResult = await this.viewBodyGenerator(_page, _db, _url, _query);
        ...
        return {
            txt: _txt,
            maxPage: bodyResult.maxPage
        };
    },
    async viewBodyGenerator(_page, _db, _url, _query) {
        var _body = "";
        if (_query == undefined)
            _query = {
                filter: {},
            ...
            for (item of _page.content) {
                ...
                if (item.type == "select") {
                    if (item.target != undefined && item.target != "") {
                        var k;
                        try {
                            k = await _db.collection(item.target).findOne({
                                _id: ObjectId(val[item.key])
                            });
                        } catch (error) {
                            continue;
                        }
                        ...
                        var re = (await _db.collection("Sabit Seçim Verileri").findOne({
                            "name": item.fixedData
                        })).content;
                        var isNull = true;
                        for (i of re) {
    ...
    async checkAllow(user, db, collection) {
    ...
    async addNotification(_db, _method, _topic, _link, _data) {
        var Users = await _db.collection("Kullanıcı Bildirim Talepleri").find({
            topics: {
                topic: _topic
            }
        }).toArray();
        if (!Users.length)
            return;
        var design = await _db.collection("Tasarımlar").findOne({
            key: _method
        });
        if (design == null)
            return;
    ...
    async viewHeaderGenerator(_page, _db, _url) {
        var _headers = "<tr>";
        for (item of _page.content) {
            if (item.tableViewable) {
                if (item.type == "select") {
                    var selectItems = [],
                        selectTxt = "<option value='' >Seçiniz..</option>";
                    if (item.target != undefined && item.target != "") {
                        var tmp = await _db.collection(item.target).find({}).toArray();
                        var re = (await _db.collection("Sayfalar").findOne({
                            'collection': item.target
